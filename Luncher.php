<?php
define('BOT_TOKEN', '227429635:AAFtcbKZ2DnIYRnTqkDOdtWGutxkLImmGhE');
...
--------------------------------
شما در ليست سياه قرار داريد ??
لطفا پيام ندهيد??" ,"parse_mode" =>"HTML"));	
    ...
    apiRequest("sendphoto", array('chat_id' => $membersidd[$y], "photo" => $substtr,"caption" =>$caption));
		...
		$memcout = count($membersidd)-1;
	 	apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "?? پيام شما به  ".$memcout." مخاطب ارسال شد.
...
if($chat_id==$admin){
  ...
  'سلام??
??به بخش مديريت ربات پيام رسان خود خوش آمديد??

??نکات  لازم :
?براي پاسخ به پيام،روي آن پيام ريپلي کرده و ارسال کنيد
?نيازي به افزودن ربات در گروهي نداريد.
?پيام هاي چند رسانه اي هم  پشتيباني ميشود(هم ارسالي و دريافتي)
?ربات بدون تبليغات ميباشد.
?هرگونه خطا,غلط املايي و يا نظرات و پيشنهادات خود را از طريق ربات زير براي ما ارسال کنيد.
@PMresan_Admin_Bot

?براي آشنايي با نحوه مديريت ربات روي دکمه Help کليک کنيد'
...
-!-@-#-$"."
".$pmembersiddd[1]);
apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" =>"??پيام خوش آمد گويي به ??
...
-!-@-#-$"."
".$starttext);
apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" =>"??پيام پيش فرض شما به ??
...
يکي از گزينه ها را انتخاب کنيد
—---------------------------------------------
???? Clean Members
????پاک کردن ليست مخاطبين

????Clean Block List
????پاک کردن ليست سياه

در صورت انصراف Back را بزنيد
.', 'reply_markup' => array(
...
,"parse_mode" =>"MARKDOWN",'reply_markup' => array(
        ...
        $pmembersid= explode("\n",$txxt);
    		...
    		apiRequestJson("sendMessage", array('chat_id' => $chat_id,"parse_mode"=>"HTML", "text" => 'ليست مخاطبين پاک شد ??
.', 'reply_markup' => array(
        'keyboard' => array(array('?? Send To All'),array('?? Help','?? Members','? Blocked Users'),array("Settings ?")),
        'one_time_keyboard' => true,
        'selective' => true,
        ...
        $pmembersid= explode("\n",$txxt);
    		...
    		apiRequestJson("sendMessage", array('chat_id' => $chat_id,"parse_mode"=>"HTML", "text" => 'ليست سياه پاک شد ? ', 'reply_markup' => array(
        'keyboard' => array(array('?? Send To All'),array('?? Help','?? Members','? Blocked Users'),array("Settings ?")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
    		...
    		apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => 'بازگشت به منوي اصلي:
', 'reply_markup' => array(
        'keyboard' => array(array('?? Send To All'),array('?? Help','?? Members','? Blocked Users'),array("Settings ?")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
          ...
          apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "?? پيام خود را ارسال کنيد ." ,"parse_mode" =>"HTML"));
      $boolean = file_get_contents('booleans.txt');
		  $booleans= explode("\n",$boolean);
...
.","parse_mode" =>"HTML",'reply_markup' => array(
		...
		$membersidd= explode("\n",$txtt);
		$mmemcount = count($membersidd) -1;
		 apiRequestJson("sendMessage", array('chat_id' => $chat_id,"parse_mode" =>"HTML", "text" => "?? تعداد کل افرادي که در ليست سياه قرار دارند : ".$mmemcount,'reply_markup' => array(
        'keyboard' => array(array('?? Send To All'),array('?? Help','?? Members','? Blocked Users'),array("Settings ?")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
...
------------------
از ليست سياه پاک شد." ,"parse_mode" =>"HTML"));
		apiRequest("sendMessage", array('chat_id' => $rpto, "text" => "<b>You Have Been UnBanned?,</b>
-----------------
شما از ليست سياه پاک شديد ?." ,"parse_mode" =>"HTML"));
    		}
    	}
	}
  } else {
...
define('WEBHOOK_URL', 'https://mm-pvresaan.rhcloud.com/Luncher.php/');
